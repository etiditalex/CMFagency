...
/**
 * Fusion Xpress dashboard: generate branded invoice PDFs.
 * Allows portal members except employers; legacy `admin_users` without `portal_members` is allowed.
 */
export async function requireFusionPortalInvoiceAccess(req: NextRequest): Promise<
  | { admin: SupabaseClient; userId: string }
  | { error: NextResponse }
> {
  const authHeader = req.headers.get("authorization");
  ...
  const { data: legacy } = await admin.from("admin_users").select("user_id").eq("user_id", callerId).maybeSingle();
  if (!legacy) {
    return { error: NextResponse.json({ error: "Forbidden: portal membership required" }, { status: 403 }) };
  }
