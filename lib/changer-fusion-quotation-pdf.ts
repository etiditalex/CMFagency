...
export type BuildChangerFusionQuotationPdfInput = {
  billToName: string;
  billToEmail?: string;
  billToPhone?: string;
...
/** Filename like `QUOTATION FOR MR. JUSTINE _20260504_153039_0000.pdf` */
export function buildQuotationPdfFilename(billToName: string, d = new Date()): string {
  const label = billToName.trim() || "CLIENT";
  const { ymd, hms } = nairobiYmdHms(d);
  const seq = String(Math.floor(Math.random() * 10000)).padStart(4, "0");
  return `QUOTATION FOR ${label} _${ymd}_${hms}_${seq}.pdf`;
}
...
function drawPaymentAndSignature(
  ...
  y -= 11;
  const payNote = `Use quotation reference ${quotationRef} as the payment description / narration where applicable.`;
...
export async function buildChangerFusionQuotationPdfBytes(
  input: BuildChangerFusionQuotationPdfInput
): Promise<Uint8Array> {
  ...
  const docTitle = (input.documentTitle ?? "Quotation").trim();
  page.drawText(docTitle.toUpperCase(), { x: M, y: y - 8, size: 20, font: fontBold, color: BRAND_DARK });
  y -= 36;

  page.drawText(`Reference ${input.quotationRef}`, { x: M, y: y, size: 10, font: fontReg, color: BRAND_GRAY });
  y -= 22;
  ...
  by -= 18;
  page.drawText(`Quotation date: ${quoteDate}`, { x: bx, y: by, size: 10, font: fontReg, color: BRAND_DARK });
  page.drawText(`Valid until: ${validUntil}`, { x: bx + 260, y: by, size: 10, font: fontReg, color: BRAND_DARK });

  y = boxTop - boxH - 28;

  page.drawText("QUOTED TO", { x: M, y: y, size: 8, font: fontBold, color: ACCENT });
  y -= 16;
  ...
  const totalLabel = "Quoted amount (KSh)";
  const totalLabelW = fontBold.widthOfTextAtSize(totalLabel, 9);
  page.drawText(totalLabel, { x: PAGE_W - M - totalLabelW - 12, y: y - 20, size: 9, font: fontBold, color: BRAND_GRAY });
  ...
  if (y < MIN_Y_BEFORE_PAYMENT_BLOCK) {
    page = doc.addPage([PAGE_W, PAGE_H]);
    y = H - M - 8;
    page.drawText(`Changer Fusions · ${input.quotationRef}`, { x: M, y, size: 9, font: fontReg, color: BRAND_GRAY });
    y -= 20;
  }

  drawPaymentAndSignature(page, fontReg, fontBold, input.quotationRef, y);
