...
const STORIES = [
  ...
  },
  {
    ...
    quote:
      "We needed an event that still worked online the next morning. Their team treated ticketing, content, and the live moment as one brief. Dummy quote for layout while we collect live stories: the run of show was clear, the campaign stayed live after last call, and we did not have to rebuild the story for social from scratch.",
  },
  {
    name: "Faith Chebet",
    ...
    quote:
      "From audition floor to the awards night, the process was clear. Dummy story: I always knew the next step, and the team treated talent with the same care they give the brand on stage. Call times, looks, and the live moment were written down, so nobody was guessing, and the show still felt personal when the lights came up.",
  },
  {
    name: "Daniel Mwangi",
    ...
    quote:
      "Placeholder testimonial: run-of-show, vendors, and guest flow stayed tight. Changer Fusions made a complex day feel simple for our staff and for the people walking in the door. Dummy copy: radios, timings, and the last-minute changes all came through one desk, which meant the floor stayed calm even when the programme shifted.",
  },
  {
    ...
    quote:
      "Dummy copy: the digital work did not stop when the event ended. Posts, tickets, and follow-up sat in one place, so the brand stayed visible after the last guest left. We could see who came, who voted, and what to send next week without opening five tools, which is the part of the brief we keep coming back to.",
  },
  {
    name: "Peter Kamau",
    role: "Events Partner",
    ...
    quote:
      "Sample story for this layout: we asked for a partner who could plan, promote, and measure. Changer Fusions did the three together, which is why we keep coming back. Dummy note: the event brief, the Fusion Xpress tickets, and the campaign report arrived as one thread, so our team spent the week on guests instead of chasing files.",
  },
] as const;
...
function StorySlot({
  ...
  return (
    <article
      className={`relative min-h-[26rem] overflow-hidden rounded-xl bg-secondary-800 sm:min-h-[28rem] lg:min-h-[32rem] ${className}`}
    >
      {STORIES.map((person) => (
        <div
          key={person.name}
          className={`absolute inset-0 flex flex-col px-4 py-5 transition-opacity ${FADE_CLASS} ease-in-out sm:px-6 sm:py-8 lg:px-7 lg:py-9 ${
        ...
        >
          <div className="mb-3 flex items-center gap-3 sm:mb-6 sm:gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="home-success-role mt-0.5 truncate text-xs text-white/80 sm:text-sm">{person.role}</p>
            </div>
          </div>
  ...
  return (
    ...
    >
      <h2
        ...
        className="mb-6 px-4 font-montserrat text-[1.75rem] font-bold text-white sm:mb-12 sm:text-4xl md:mb-14 md:text-5xl"
      >
        Success Stories
      </h2>
      ...
      <div
        className="mx-auto grid w-full grid-cols-1 gap-4 px-4 sm:grid-cols-3 sm:gap-4 sm:px-6 md:px-8 lg:gap-5 lg:px-10"
        aria-hidden="true"
      >
        {(reduceMotion ? STORIES.slice(0, VISIBLE) : visible).map((story, slot) => (
          <StorySlot
            ...
            story={story}
            className={slot > 0 ? "hidden sm:block" : undefined}
          />
        ))}
      </div>
    </section>
