...
import { Search, ShoppingCart, Heart, Star, Check, X } from "lucide-react";
...
export default function MerchandiseClient({ initialMerchandise }: { initialMerchandise: MerchItem[] }) {
  ...
  useEffect(() => {
    // If server already gave us items, avoid an immediate refetch.
    if (initialMerchandise.length > 0) {
      setVariantById((prev) => {
        const next = { ...prev };
        for (const it of initialMerchandise) {
          if (next[it.id]) continue;
          next[it.id] = { size: it.sizes[0] ?? "", color: it.colors[0] ?? "" };
        }
        return next;
      });
      return;
        ...
        <div className="container-custom">
          {catalogError && (
            <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900 text-sm">
              ...
              <p className="mt-1 text-amber-800/90">{catalogError}</p>
            </div>
          )}
            ...
            {!loadingCatalog &&
              filteredMerchandise.map((item, index) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="mb-2">
                      <span className="text-xs font-semibold text-primary-600 uppercase">{item.category}</span>
                    </div>
                    <h3 className="text-lg font-bold mb-2 text-gray-900 group-hover:text-primary-600 transition-colors">
                      <Link href={`/merchandise/${item.id}`} className="hover:underline">
                        {item.name}
                      </Link>
                    </h3>
                    ...
                    <Link
                      ...
                      className="inline-flex text-xs font-semibold text-primary-700 hover:underline"
                    >
                      View details
                    </Link>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center space-x-2">
                          <span className="text-2xl font-bold text-gray-900">KSh {item.price.toLocaleString()}</span>
                    ...
                    <button
                      ...
                      className={`w-full mt-4 py-3 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center space-x-2 ${
                        ...
                        !(
                          ...
                          ? cart.some(
                              (cartItem) =>
                                cartItem.key === `${item.id}::${variantById[item.id]?.size ?? ""}::${variantById[item.id]?.color ?? ""}`
                            )
          ...
          {!loadingCatalog && filteredMerchandise.length === 0 && !catalogError && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
              <p className="text-gray-600 text-lg">
                {merchandise.length === 0 ? "No products are available yet." : "No products found matching your criteria."}
              </p>
            </motion.div>
          )}
        </div>
          ...
          >
            <div className="relative mx-auto h-full w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="relative h-full w-full">
                <Image src={lightbox.src} alt={lightbox.alt} fill unoptimized className="object-contain" />
              </div>
            </div>
          </motion.div>
        )}
