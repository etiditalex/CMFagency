...
export async function POST(req: NextRequest) {
  ...
  let payload: InvoicePdfRequestBody;
  try {
    payload = (await req.json()) as InvoicePdfRequestBody;
  } catch {
  ...
  const invoiceRef = `CF-${y}-${String(Math.floor(Math.random() * 900000) + 100000)}`;
  ...
  const filename = buildInvoicePdfFilename(safeLabel || "CLIENT", now);
