...
export default function DashboardQuotationsPage() {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/fusion-xpress/quotations/pdf", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          documentTitle: documentTitle.trim() || "Quotation",
      ...
      if (!res.ok) {
        ...
        throw new Error((j as { error?: string }).error ?? "Could not generate quotation");
      }
      ...
      a.href = url;
      a.download = parseFilenameFromDisposition(res.headers.get("Content-Disposition")) ?? "quotation.pdf";
      a.rel = "noopener";
    ...
    <div className="space-y-8">
      <div>
        <p className="text-sm text-gray-600 leading-relaxed max-w-2xl">
          Generate a branded Changer Fusions quotation PDF in one click. The file name follows your client name and
          Nairobi date/time, for example{" "}
          <span className="font-mono text-xs text-gray-800 bg-gray-100 px-1.5 py-0.5 rounded">
            QUOTATION FOR MR. JUSTINE _20260504_153039_0000.pdf
          </span>
          .
        </p>
      </div>
        ...
        <div className="text-sm text-gray-700 leading-relaxed min-w-0">
          ...
          <div className="mt-3 text-xs text-gray-700 leading-relaxed border-t border-gray-200 pt-3">
            ...
            <span className="block mt-1 text-gray-600">
              Use the quotation reference shown on the PDF as the payment description. Printed PDFs include a signature
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wide text-gray-500">Quote to name *</label>
            <input
              value={billToName}
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wide text-gray-500">Valid until</label>
            <input
              type="date"
              value={dueDate}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <FilePenLine className="w-4 h-4" />}
          Download PDF
        </button>
        {!canSubmit ? <span className="text-xs text-gray-500">Enter quote-to name and at least one valid line.</span> : null}
      </div>
    </div>
  );
}
