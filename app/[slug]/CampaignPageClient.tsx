...
import type { CampaignPageData } from "@/lib/campaign-page-data";
...
const COUNTDOWN_LABELS = ["MONTHS", "DAYS", "HRS", "MIN", "SEC"] as const;

function VotingEndsCountdown({ show, targetMs }: { show: boolean; targetMs: number }) {
  /**
   * Stays null through the server render and first paint: the page is server-rendered now, so
   * seeding from `Date.now()` would hydrate against a clock that has already moved on.
   */
  ...
  const t = nowMs === null ? null : computeCountdown(new Date(nowMs), new Date(targetMs));
  const done = t !== null && t.months + t.days + t.hours + t.minutes + t.seconds <= 0;
  ...
  const values =
    t === null
      ...
      : done
        ? ["00", "00", "00", "00", "00"]
        : [String(t.months), String(t.days), pad2(t.hours), pad2(t.minutes), pad2(t.seconds)];

  const items = COUNTDOWN_LABELS.map((label, i) => ({ label, value: values[i] }));
...
function resolveInitialState(data: CampaignPageData | null): InitialState {
  const campaign = data && !data.not_found && data.campaign ? (data.campaign as Campaign) : null;
  const isVote = campaign?.type === "vote";

  const startedAt = data?.voting_starts_at ? Date.parse(data.voting_starts_at) : NaN;

  return {
    campaign,
    contestants: isVote ? data?.contestants ?? [] : [],
    voteCounts: isVote ? data?.vote_counts ?? {} : {},
    showVoteTotals: data?.show_vote_totals !== false,
    votingStartMs: Number.isNaN(startedAt) ? FALLBACK_VOTING_START_MS : startedAt,
    votingEndMs: votingEndMsFromSchedule(data?.voting_ends_at ?? null),
  };
}

/**
 * Server-fetched props replace the old mount-time `/page-data` request, so the campaign
 * renders in the first paint instead of after hydration plus a client round-trip.
 * The server page remounts this via `key={slug}`, so state never leaks between campaigns.
 */
export default function CampaignPageClient({
  ...
  slug: string;
  initialData: CampaignPageData | null;
  initialError: string | null;
  /** Server clock at render time; the open/closed gates read this so hydration matches. */
  serverNowMs: number;
}) {
  ...
  const initial = useMemo(() => resolveInitialState(initialData), [initialData]);

  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(
    initialError ?? (initial.campaign ? null : GENERIC_CAMPAIGN_LOAD_FAILURE)
  );
  const receiptRequestedRef = useRef(false);
  const reminderRequestedRef = useRef(false);
  const voteSuccessToastFiredRef = useRef(false);

  const [campaign] = useState<Campaign | null>(initial.campaign);
  const [contestants] = useState<Contestant[]>(initial.contestants);
  const [voteCounts, setVoteCounts] = useState<Record<string, number>>(initial.voteCounts);
  ...
  const votingStartMs = initial.votingStartMs;
  const votingEndMs = initial.votingEndMs;
  /** Closes/opens the page the moment the countdown crosses over, without waiting for a refresh. */
  const [votingClosed, setVotingClosed] = useState(() => serverNowMs >= votingEndMs);
  const [votingNotYetOpen, setVotingNotYetOpen] = useState(() => serverNowMs < votingStartMs);
  /** Global admin switch; when off, tallies and vote-based ranking stay hidden from voters. */
  const [showVoteTotals, setShowVoteTotals] = useState(initial.showVoteTotals);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const now = Date.now();
      setVotingClosed(now >= votingEndMs);
      setVotingNotYetOpen(now < votingStartMs);
    };
    tick();
    const id = window.setInterval(tick, 1000);
    return () => window.clearInterval(id);
  }, [votingEndMs, votingStartMs]);
  ...
  const isVote = campaign.type === "vote";
  const votingLocked = isVote && votingNotYetOpen;
      ...
      <div className="container-custom py-10 max-w-5xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
            {campaign.image_url && (
              <div className="mb-6 -mx-6 -mt-6 rounded-t-xl overflow-hidden relative h-48 sm:h-56">
                <Image
                  ...
                  unoptimized={!canOptimizeRemoteImage(campaign.image_url)}
                />
              </div>
            )}
            <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {showVoteTotals && (
                              <div className="text-sm font-semibold text-primary-600 mt-0.5">
                                {votes.toLocaleString("en-KE")} vote{votes !== 1 ? "s" : ""}
                              </div>
                            )}
            ...
            <form onSubmit={onPay} className="mt-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-2">
                    {campaign.type === "vote" ? "No maximum — buy as many votes as you want (up to 1,000,000)" : `Max per transaction: ${effectiveMax.toLocaleString("en-KE")}`}
                  </p>
                </div>
                <div className="rounded-lg bg-gray-50 border border-gray-100 p-4">
                  <div className="border-t border-gray-200 pt-2">
                    ...
                    <div className="text-2xl font-bold text-gray-900 mt-1">
                      {campaign.currency} {total.toLocaleString("en-KE")}
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 space-y-3">
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-800 leading-snug">
                      ...
                      {total.toLocaleString("en-KE")} in one payment.
                    </span>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input
